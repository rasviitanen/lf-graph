#![allow(dead_code)]
use crate::lftt::{Desc, NodeDesc, OpStatus, OpType, Operator, ReturnCode};
use crate::mdlist::{MDList, MDNode};
...
impl<'a: 't, 't, T: 'a, E: 'a> Clone for RefEntry<'a, 't, T, E> {
    fn clone(&self) -> RefEntry<'a, 't, T, E> {
        RefEntry {
            node: self.node,
        }
    }
...
impl<'a: 't + 'g, 't, 'g, T: 'a, E: 'a> Iterator
...
impl<'a: 'd + 'g, 'd, 'g, T: 'a, E: 'a> AdjacencyList<'a, T, E> {
    // Public operations
    pub fn new() -> Self {
        let guard = &epoch::pin();
...
impl<'a, T, E> AdjacencyList<'a, T, E> {
